...
find_cycle_length :: Map [Int] Int -> [Int] -> Int
find_cycle_length my_map ls = (Map.size my_map) - (fromJust $ Map.lookup ls my_map)

find_cycle :: State -> Map [Int] Int -> (Map [Int] Int, Int)
find_cycle state_in my_map
    | (Map.member ls my_map) = (my_map, (find_cycle_length my_map ls))
...
main = do
    ...
    print $ snd $ find_cycle (State memory 0 0 0) (Map.empty)
